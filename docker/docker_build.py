...
if __name__=="__main__":
	...
	docker_build_root = os.path.dirname(cwd)


	print("docker_build_root_dir:", docker_build_root)
	...
	cmd = "cd %(docker_build_root_dir)s && docker build --build-arg USER_NAME=%(user_name)s \
			--build-arg USER_PASSWORD=%(password)s \
			--build-arg USER_ID=%(user_id)s \
			--build-arg USER_GID=%(group_id)s" \
			%{'docker_build_root_dir': docker_build_root,
			  'user_name': user_name,
	...
	cmd += " -t %(image)s -f %(docker_build_root)s/docker/env.dockerfile ." \
		   % {'image': args.image, 'docker_build_root': docker_build_root}
